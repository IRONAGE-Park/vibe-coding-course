...
- `바이브 코딩과 함께 살아남기_YYYYMMDD.pdf` — **발표용 PDF**, 내보낸 날짜별로 따로 보관 (최신 `_20260910_v2`: 52장, 링크 클릭 가능 — AI 네이티브 SDLC 3장 추가 · 단정적 표현 정리 · '오늘 하루에 끝나는 과제' 삭제. 이전본 `_20260910`: 51장)

- `session1-slides.html` — 1교시 강의 슬라이드 "바이브 코딩과 함께 살아남기" (16:9, 1280×720, 52장, 페이지 번호는 CSS 카운터 자동, 빈칸 없음 — 발표 준비 완료)
  ...
  - 예시에 나온 서비스는 카드 안에 바로 가는 링크(`.svc`, PDF에서 클릭 가능): 14p 서울시 상권분석서비스 · 21p 몰트북 · 42p 벚꽃지도 · 43p 러브버그.com / 거지맵.com / 야장맵.kr · 44p base44.com
  - 13~19p: 연습문제 3세트 (질문→판단: 데이터 해상도 / 이용 권한 / 현실의 규칙) + "세 번 다 다른 곳에서 막혔습니다" 정리 — 창업팀 맥락으로 교체(2026-09-10), 근거는 `docs/research/` 참고
  - 20~21p: 위험 2장 — "이런 경우에 자주 문제가 됩니다"(대표 예시: 방법 자체 위법 / 남의 자료 / 정보 유출 / 과장 광고 — 이것뿐이라고 단정하지 않음) + "실제로 있었던 사례" 3건 (2026-09-10 신규, 같은 날 표현 정리)
  - 26~30p: 개발 프로세스 5장 — 26p 간지 → 27p Waterfall / Agile → 28p "코드는 더 이상 병목이 아닙니다"(기존 SDLC vs AI 네이티브 SDLC) → 29p "AI 네이티브 SDLC — 6단계 루프"(Plan intent.md · Design spec.md · Build 구현 계획 · Test 자가 검증 · Deploy AI 리뷰·승인 게이트 · Maintain 새 intent.md, 단계마다 사람의 몫) → 30p "바이브 코딩에 어울리는 프로세스는?"(루프를 오늘 실습 크기로). 28~29p는 Anthropic 「The AI-Native SDLC Playbook」(2026-08-21, claude.com/blog/the-ai-native-sdlc-playbook)과 Claude Academy 동명 코스 기준 (2026-09-10 신규)
  - 41~47p: 사례 7장 (국내 2건 캡처 + 해외 1건 + 현실 점검 / 측정 질문 / 아이디어를 좋은 문제로) — 창업팀 맥락으로 교체(2026-09-10), 근거는 `docs/research/` 참고. '오늘 하루에 끝나는 크기의 과제'는 삭제
  - 48~50p: 에이전트 활용 3장 (2026-09-10 신규) — 48p 간지 "에이전트를 더 잘 쓰는 방법" → 49p "만들지 않고 맡기기"(자료 수집·조사 / 자료 가공 / 문서·PPT, 인기 도구: Deep Research · 서브에이전트 · Claude Cowork · Claude for Excel/PowerPoint · pptx 스킬 · ppt-master) → 50p "남이 만든 일하는 요령, 스킬"(GitHub 저장소 7개 이름·설명 — 다이어그램 스킬 archify·diagram-design, 한국어 k-skill 포함 + 권한 주의)
  - 52p: 실습 안내 + 2교시 가이드 URL (vibe-coding-course-kappa.vercel.app)
